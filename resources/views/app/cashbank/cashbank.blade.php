        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-blue-sharp bold uppercase">Cash</span>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="portlet box blue-chambray">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-globe"></i>List of Cash
                            </div>
                            <div class="tools">
                        ...
                        <div class="portlet-body"> 
                            <div class="table-toolbar">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="list_cashbank">
                                <thead>
                                    <tr>  
                                        <th>Bank Account</th>
                                        <th>Bank Account</th>
...
<!-- PROPERTIES HERE ALL -->
<!-- create_modal modal-->
<div id="create_modal" class="modal fade bs-modal-lg" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                <h4 class="modal-title">Cash Transaction</h4>
            </div>
                ...
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-red-sunglo">
                            ...
                            <span class="caption-subject bold uppercase">Set Cash</span>
                        </div> 
                    </div>
                    <!-- modal form add -->
                    <div class="portlet-body form">
                        <form role="form" id="create_cashbank_form"> 
                            {{ csrf_field() }}
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-md-12">
            ...
            <div class="modal-body">     
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-red-sunglo">
                            <i class="icon-settings font-red-sunglo"></i>
                            <span class="caption-subject bold uppercase">Edit Cash</span>
                        </div> 
                    </div>
                    <!-- modal form update -->
                    <div class="portlet-body form">
                        <form role="form" id="update_cashbank_form"> 
                            {{ csrf_field() }}
                            <input type="hidden" name="cashbank_id">
                            <div class="form-body">
                               <div class="form-group form-md-line-input">  
                                ...
                                <div class="form-group form-md-line-input"> 
                                    <select class="form-control bank-account-form" name="bank_account_id" > 
                                        @foreach($listBank as $bank)
                                            @foreach($listBankAccount as $bankAccount)
                                                @if($bank->bank_id === $bankAccount->bank_id)
                                                    <option value="{{$bankAccount->bank_account_id}}">{{$bank->bank_name}}&nbsp{{$bankAccount->account_no}}&nbsp({{$bankAccount->account_detail}})</option>
                                                @endif
                                            @endforeach
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group form-md-line-input">  
                                    <label class="month_request_label">{{$nowMonthString}}</label>
                                    <input type="hidden" class="form-control month_request" name="month" readonly="readonly"> 
                                    <label>Month</label> 
                                </div> 
                                <div class="form-group form-md-line-input"> 
                                    <input type="text" class="form-control week_request" name="week" readonly="readonly">
                                    <label>Week</label> 
                                </div>  
                            </div>
                            <div class="form-actions noborder">
...
<div id="export_modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{url('export/excel/cashbank/')}}">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
...
<script> 
    ...
    $(function(){ 
        ...
        // set for set new modal And Copy data value from datatable
        $("#set_new_button").on("click", function(){ 
            var isEmptyTable = $("#list_cashbank").children("tbody").find("td:first").hasClass('dataTables_empty') ; 
            if(isEmptyTable){ 
                $("#create_modal").modal('show');
            } else{
                $("#set_new_dialog").modal('show');
                $(".act_set_new").on("click", function(){
                    $('#list_cashbank tbody tr').each(function() {
                        var id  = $(this).find(".ba_id").val();
                        var in_  = $(this).children("td:nth-child(3)").text();
                        var out_ = $(this).children("td:nth-child(4)").text();  
...
//                    {data: 'date_transaction', name: 't_cashbank.date_transaction',
...
//                    },  
        ...
        //autocomplete email
        var path = "{{url('/autocompleteEmail/cashbank')}}";
        $('.typeahead').typeahead({  
        ...
        $('#view_year_selected,#view_project_selected,#view_operational_type_selected').change(function (e) { 
            var projectSelected     = $("#view_project_selected").val();   
        ...
        $('#view_project_selected,#view_year_selected,#view_month_selected,#view_week_selected').change(function (e) {  
             ...
             var weekSelected      = $("#view_week_selected").val();  
             $.ajax({
                   'url'  : "{{ url('/cashbank/gettotalvalue')}}"+"/"+projectSelectedID+"/"+yearSelected+"/"+monthSelected+"/"+weekSelected,
                   'type' : 'GET',
		...
		@if(count($listProject)>1)
            ...
            $('#view_project_selected').change(function (e) {    
                 var projectSelectedID = $("#view_project_selected").val();  
                 $.ajax({
                       'url'  : "{{ url('/cashbank/listbankaccount') }}" + '/' + projectSelectedID,
                       'type' : 'GET',
                            ...
                            for(var i = 0;i<bank.length;i++){
                                for(var j = 0;j<bankAccount.length;j++){
                                    if(bank[i]['bank_id'] === bankAccount[j]['bank_id']){
                                        htmlselect += '<option value="'+bankAccount[j]['bank_account_id']+'">'+bank[i]['bank_name']+'&nbsp'+bankAccount[j]['account_no']+'&nbsp('+bankAccount[j]['account_detail']+')</option>';
                                    }   
                                }
                            }
        ...
        $form = $("#create_cashbank_form"); 
        $(document).on('click','.btn_show', function(){ 
            var id = $(this).data('id');
            $.ajax({
               'url' : "{{url('cashbank/show')}}",
                data : {
                    id : id
                },
                success : function(result){
        ...
        //update
        $formUpdate = $("#update_cashbank_form");
        ...
        //delete action
        $('#list_cashbank').on('click', '.btn_delete', function (e) {  
