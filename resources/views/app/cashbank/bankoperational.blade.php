...
<div class="page-bar">
    <ul class="page-breadcrumb">
        <li>
            ...
            <a href="{{url('/bankoperational')}}">Bank Operational</a>
        </li> 
    </ul>
</div>
        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-blue-sharp bold uppercase">Bank Operational</span>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="portlet box blue-chambray">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-globe"></i>List of Bank Operational
                            </div>
                            <div class="tools">
                        ...
                        <div class="portlet-body"> 
                            <div class="table-toolbar">
								...
								<div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="list_bank_operational">
                                <thead>
                                    <tr>  
                                        <th>Bank Account</th>
                                        <th>Bank Account</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Bank Operational Transaction</h4>
            </div>
                ...
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-red-sunglo">
                            ...
                            <span class="caption-subject bold uppercase">Add Bank Operational</span>
                        </div> 
                    </div>
                    <!-- modal form add -->
                    <div class="portlet-body form">
                        <form role="form" id="create_bank_operational_form"> 
                            {{ csrf_field() }}
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-md-12">
            ...
            <div class="modal-body">     
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-red-sunglo">
                            <i class="icon-settings font-red-sunglo"></i>
                            <span class="caption-subject bold uppercase">Edit Bank Operational</span>
                        </div> 
                    </div>
                    <!-- modal form update -->
                    <div class="portlet-body form">
                        <form role="form" id="update_bank_operational_form"> 
                            {{ csrf_field() }}
                            <input type="hidden" name="bank_operational_id">
                            <div class="form-body">
                               <div class="form-group form-md-line-input"> 
...
<div id="export_modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{url('export/excel/bankoperational/')}}">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                ...
                <div class="modal-body">
                    <div class="scroller" style="height:450px" data-always-visible="1" data-rail-visible1="1">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <h4>Operational Type</h4>
                                <p>
                                    <select class="col-md-12 form-control" name="operational_type">
                                        <option value="bank_operational">Bank Operational</option> 
                                        <option value="bank_operational_jo">Bank Operational - JO</option> 
                                        <option value="rekber_jo">Rekber - JO</option> 
                                    </select>
                                </p> 
                            </div>
                            <div class="col-md-12">
...
<div id="mail_modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{url('sendmail/bankoperational/')}}" id="sendmail_form">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
...
<script>
    ...
    $(function(){
        ...
        $("#set_new_button").on("click", function(){   
            var isEmptyTable = $("#list_bank_operational").children("tbody").find("td:first").hasClass('dataTables_empty') ;  
            if(isEmptyTable){ 
                $("#create_modal").modal('show');
            } else{
                $("#set_new_dialog").modal('show');
                $(".act_set_new").on("click", function(){
                    $('#list_bank_operational tbody tr').each(function() {
        ...
        grid.init({
            ...
            dataTable: { 
                ...
                "ajax": {
                    "url":  "{{url('bankoperational/datatable')}}",
                    "type"  : "POST" ,
                    data: function(d){ 
                 ...
                 columns: [   
                    ...
                    {data: 'operational_type', name: 'operational_type'},     
        ...
        var path = "{{url('/autocompleteEmail/bankoperational')}}";
        $('.typeahead').typeahead({  
        ...
        $('#view_project_selected,#view_year_selected,#view_month_selected,#view_week_selected').change(function (e) {  
             ...
             var weekSelected      = $("#view_week_selected").val();  
             $.ajax({
                   'url'  : "{{ url('/bankoperational/gettotalvalue')}}"+"/"+projectSelectedID+"/"+yearSelected+"/"+monthSelected+"/"+weekSelected,
                   'type' : 'GET',
		...
		@if(count($listProject)>1)
            ...
            $('#view_project_selected').change(function (e) {    
                 var projectSelectedID = $("#view_project_selected").val();  
                 $.ajax({
                       'url'  : "{{ url('/bankoperational/listbankaccount') }}" + '/' + projectSelectedID,
                       'type' : 'GET',
        ...
        $form = $("#create_bank_operational_form"); 
        $(document).on('click','.btn_show', function(){
            var id = $(this).data('id');
            
            $.ajax({
               'url' : "{{url('bankoperational/show')}}",
                data : {
                    id : id
                },
                success : function(result){
        ...
        $("#submit",$form).click(function(){
           ...
           $.ajax({
               'url' : "{{url('bankoperational/create')}}",
               'type' : 'POST',
               'data' : $form.serialize(),
               'success' : function(result, textStatus, xhr){
                   console.log(result);
        ...
        //update
        $formUpdate = $("#update_bank_operational_form");
        ...
        //delete 
        $('#list_bank_operational').on('click', '.btn_delete', function (e) {  
            ...
            $(".act_delete").on("click", function(){ 
                ...
                // confirm then
                $.ajax({
                    url: '{{ url('/bankoperational/') }}' + '/' + dataID,
                    type: 'DELETE',
                    data: dataID,
