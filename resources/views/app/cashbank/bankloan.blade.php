...
<div class="page-bar">
    <ul class="page-breadcrumb">
        <li>
            ...
            <a href="{{url('/bankloan')}}">Bank Loan</a>
        </li> 
    </ul>
</div>
        ...
        <div class="portlet light ">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject font-blue-sharp bold uppercase">Bank Loan</span>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="portlet box blue-chambray">
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-globe"></i>List of Bank Loan
                            </div>
                            <div class="tools">
                        ...
                        <div class="portlet-body"> 
                            <div class="table-toolbar">
                                        ...
                                        <div class="btn-group pull-right" style="margin:5px 3px;">
                                            <select class="table-group-action-input form-control input-inline input-sm" id="view_week_selected"> 
                                                ...
                                                @foreach($listWeek as $w)
                                                    <option value="{{$week}}">Week {{$week++}}&nbsp({{$w}}&nbsp{{$nowMonthString}})</option>  
                                                @endforeach
                                            </select>  
										</div> 
										<div class="btn-group pull-right" style="margin:5px 3px;">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="list_bankloan">
                                <thead>
                                    <tr>  
                                        <th>Bank Account</th>
                                        <th>Bank Account</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Bank Loan Transaction</h4>
            </div>
                ...
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-red-sunglo">
                            ...
                            <span class="caption-subject bold uppercase">Add Bank Loan</span>
                        </div> 
                    </div>
                    <!-- modal form add -->
                    <div class="portlet-body form">
                        <form role="form" id="create_bankloan_form"> 
                            {{ csrf_field() }}
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-md-12">
            ...
            <div class="modal-body">     
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-red-sunglo">
                            <i class="icon-settings font-red-sunglo"></i>
                            <span class="caption-subject bold uppercase">Edit Bank Loan</span>
                        </div> 
                    </div>
                    <!-- modal form update -->
                    <div class="portlet-body form">
                        <form role="form" id="update_bankloan_form"> 
                            {{ csrf_field() }}
                            <input type="hidden" name="bankloan_id">
                            <div class="form-body">
                               <div class="form-group form-md-line-input"> 
...
<div id="export_modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{url('export/excel/bankloan/')}}">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
...
<div id="mail_modal" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{url('sendmail/bankloan/')}}" id="sendmail_form">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
...
<script>
    ...
    $(function(){
        ...
        $("#set_new_button").on("click", function(){   
            var isEmptyTable = $("#list_bankloan").children("tbody").find("td:first").hasClass('dataTables_empty') ;  
            if(isEmptyTable){ 
                $("#create_modal").modal('show');
            } else{
                $("#set_new_dialog").modal('show');
                $(".act_set_new").on("click", function(){
                    $('#list_bankloan tbody tr').each(function() {
        ...
        var path = "{{url('/autocompleteEmail/bankloan')}}";
        $('.typeahead').typeahead({  
        ...
        $('#view_project_selected,#view_year_selected,#view_month_selected,#view_week_selected').change(function (e) {  
             ...
             var weekSelected      = $("#view_week_selected").val();  
             $.ajax({
                   'url'  : "{{ url('/bankloan/gettotalvalue')}}"+"/"+projectSelectedID+"/"+yearSelected+"/"+monthSelected+"/"+weekSelected,
                   'type' : 'GET',
                   'success' : function(result, textStatus, xhr){   
                         $("#total_balance_before").text("Total Balance Before : "+result[0]['totalBefore']);
                         $("#total_in").text('Total In : '+result[0]['totalIn']);
                         $("#total_out").text('Total Out : '+result[0]['totalOut']);
                         $("#total_balance_after").text("Total Current Balance : "+result[0]['totalAfter']);
                   }
             });
        });
		@if(count($listProject)>1)
            ...
            $('#view_project_selected').change(function (e) {    
                 var projectSelectedID = $("#view_project_selected").val();  
                 $.ajax({
                       'url'  : "{{ url('/bankloan/listbankaccount') }}" + '/' + projectSelectedID,
                       'type' : 'GET',
        ...
        $form = $("#create_bankloan_form"); 
        $(document).on('click','.btn_show', function(){
            var id = $(this).data('id');
            
            $.ajax({
               'url' : "{{url('bankloan/show')}}",
                data : {
                    id : id
                },
                success : function(result){
        ...
        //update
        $formUpdate = $("#update_bankloan_form");
        ...
        //delete 
        $('#list_bankloan').on('click', '.btn_delete', function (e) {  
